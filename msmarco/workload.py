...
class MsMarcoVectorSearchRunner:
    """
    Custom runner that executes the query built by MsMarcoSearchPartition,
    extracts the returned document IDs, and computes Recall against the ground truth.
    """
    def __init__(self):
        pass
    ...
    async def __call__(self, es, params):
        """
        es: The OpenSearch client instance
        params: The payload dictionary generated by your MsMarcoSearchPartition
        """
        index_name = params["index"]
        body = params["body"]
        
        # 1. Execute the search query against OpenSearch
        start_time = time.perf_counter()
        response = await es.search(index=index_name, body=body)
        duration = time.perf_counter() - start_time
        
        # 2. Extract returned internal document IDs from the response hits
        # Assumes document IDs were indexed as strings representing their file positions
        returned_ids = []
        ...
        # 3. Initialize base metrics dictionary
        metrics = {
            "weight": 1,
            "unit": "ops",
            "success": True
        ...
        # 4. Extract ground truth pass-through data injected by your partition
        # We need to extract the ground truth indices array passed through the parameters
        gt_indices = params.get("ground_truth_indices")
        k = params.get("k", len(returned_ids) if returned_ids else 10)

        if gt_indices:
            # Take only the top-K relevant elements from the ground truth
            true_set = set(gt_indices[:k])
            returned_set = set(returned_ids[:k])
            
            true_positives = 0
            
            if len(true_set) > 0:
                # Calculate intersection (how many true positives did we find?)
                true_positives = len(true_set.intersection(returned_set))
                recall_at_k = true_positives / len(true_set)
            else:
                recall_at_k = 0.0
                
            # Append custom statistics to be aggregated by the benchmark runner
            metrics["stats"] = {
                f"recall_at_{k}": recall_at_k,
                "true_positives": true_positives,
                "expected_positives": len(true_set)
            }
